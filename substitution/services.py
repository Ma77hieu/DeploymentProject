...
class Services():
    ...
    def details(self, request, product_id):
        ...
        # following line is useful mainly for testing
    ...
    def get_rating(self, request, rated_product):
        """Allow the user to select a rating for the displayed product"""
        if request.method == "POST":
            ...
            # print('all ratings of the product ID {}: {}'.format(
            #     rated_product.id, all_ratings_of_product))
            avg_product_rating = sum(
                all_ratings_of_product)/len(all_ratings_of_product)
            # print('AVERAGE rating of the product: {}'.format(avg_product_rating))
            product_to_update = Product.objects.get(pk=rated_product.id)
            # print('name of the product to be updated: {}'.format(
            #     product_to_update.product_name))
            product_to_update.average_rating = avg_product_rating
            product_to_update.save()
            # print('new avg rating of product: {}'.format(
            #     product_to_update.average_rating))
        form = RatingForm()
        return form
